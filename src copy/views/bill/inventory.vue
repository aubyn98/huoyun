    ...
    <div class="bottomBtn-right">
      ...
      >
        ...
        <el-dropdown-menu slot="dropdown" v-if="!$store.state.mobile">
          <el-dropdown-item @click.native="print('inventory','ShowReport')">预览</el-dropdown-item>
          <el-dropdown-item @click.native="print('inventory','DesignReport')" v-if="$store.state.account && $store.state.account.administrator">编辑模板</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
...
export default {
  ...
  created() {
    if (!this.id) {
      this.msg.creator = this.$store.state.account && this.$store.state.account.userName
    }
    if (this.dataType === '列表') {
      this.$req(
        '/inventory/findByDh',
  ...
  data() {
    return {
      ...
      initMsg: {
        ...
        startStation: this.$store.state.account && this.$store.state.account.position,
        endStation: '',
        carNumber: '',
        driver: '',
        register: '',
